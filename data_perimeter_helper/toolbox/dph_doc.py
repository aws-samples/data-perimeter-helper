...
WHERE_DOCUMENTATION = {
    ...
    "AND eventsource = 'secretsmanager.amazonaws.com'": "Keep only Secrets Manager API calls.",
    "AND TRY(SPLIT(JSON_EXTRACT_SCALAR(requestparameters, '$.secretId'), ':')[5]) NOT IN ({list_all_account_id})": "Remove API calls on secrets owned by accounts belonging to the same AWS organization as the selected account.",
    "AND eventsource = 'logs.amazonaws.com'": "Keep only CloudWatch Logs API calls.",
    "AND eventname = 'PutSubscriptionFilter'": "Keep only the `PutSubscriptionFilter` event.",
    "AND TRY(SPLIT(JSON_EXTRACT_SCALAR(requestparameters, '$.destinationArn'), ':')[5]) NOT IN ({list_all_account_id})": "Remove API calls on CloudWatch destinations owned by accounts belonging to the same AWS organization as the selected account.",
    "{resource_perimeter_trusted_logs_destination_arn}": "Remove API calls made on trusted CloudWatch Logs destination tables - retrieved from the `data perimeter helper` configuration file (`resource_perimeter_trusted_ecr_repository_arn` parameter).",
    "{resource_perimeter_trusted_dynamodb_table_arn}": "Remove API calls made on trusted DynamoDB tables - retrieved from the `data perimeter helper` configuration file (`resource_perimeter_trusted_dynamodb_table_arn` parameter).",
    "AND unnested_resources.accountid NOT IN ({list_all_account_id})": "Remove API calls on resources owned by accounts belonging to the same AWS organization as the selected account.",
    "AND eventsource = 'dynamodb.amazonaws.com'": "Keep only DynamoDB API calls.",
    "{resource_perimeter_trusted_ecr_repository_arn}": "Remove API calls made on trusted ECR repositories - retrieved from the `data perimeter helper` configuration file (`resource_perimeter_trusted_ecr_repository_arn` parameter).",
    "AND eventsource = 'ecr.amazonaws.com'": "Keep only ECR API calls.",
    "{resource_perimeter_trusted_bus_arn}": "Remove API calls made on trusted bus - retrieved from the `data perimeter helper` configuration file (`resource_perimeter_trusted_bus_arn` parameter).",
    "AND TRY(SPLIT(COALESCE(JSON_EXTRACT_SCALAR(requestparameters, '$.eventBusName'),JSON_EXTRACT_SCALAR(requestparameters, '$.resourceARN')), ':')[5]) NOT IN ({list_all_account_id})": "Remove API calls on bus owned by accounts belonging to the same AWS organization as the selected account.",
    "AND eventsource = 'events.amazonaws.com'": "Keep only events API calls",
    "{resource_perimeter_trusted_kms_key_arn}": "Remove API calls made on trusted keys - retrieved from the `data perimeter helper` configuration file (`resource_perimeter_trusted_kms_key_arn` parameter).",
    "AND eventsource = 'kms.amazonaws.com'": "Keep only KMS API calls.",
    "{resource_perimeter_trusted_lambda_arn}": "Remove API calls made on trusted Lambda resources ARNs - retrieved from the `data perimeter helper` configuration file (`resource_perimeter_trusted_lambda_arn` parameter).",
    "AND TRY(SPLIT(COALESCE(JSON_EXTRACT_SCALAR(requestparameters, '$.resource'), JSON_EXTRACT_SCALAR(requestparameters, '$.functionName'), JSON_EXTRACT_SCALAR(requestparameters, '$.layerName')), ':')[5]) NOT IN ({list_all_account_id})": "Remove API calls on lambda resources owned by accounts belonging to the same AWS organization as the selected account.",
    "AND eventsource = 'lambda.amazonaws.com'": "Keep only Lambda API calls.",
    "{resource_perimeter_trusted_secret_arn}": "Remove API calls made on trusted secrets - retrieved from the `data perimeter helper` configuration file (`resource_perimeter_trusted_secret_arn` parameter).",
    "AND TRY(SPLIT(JSON_EXTRACT_SCALAR(requestparameters, '$.topicArn'), ':')[5]) NOT IN ({list_all_account_id})": "Remove API calls on topics owned by accounts belonging to the same AWS organization as the selected account.",
    "{resource_perimeter_trusted_topic_arn}": "Remove API calls made on trusted topics - retrieved from the `data perimeter helper` configuration file (`resource_perimeter_trusted_topic_arn` parameter).",
    "{resource_perimeter_trusted_sqs_queue_arn}": "Remove API calls made on trusted sqs queues - retrieved from the `data perimeter helper` configuration file (`resource_perimeter_trusted_sqs_queue_arn` parameter).",
    "AND eventsource = 'sqs.amazonaws.com'": "Keep only SQS API calls.",
    "{resource_perimeter_trusted_iam_role_arn}": "Remove API calls made on trusted IAM roles - retrieved from the `data perimeter helper` configuration file (`resource_perimeter_trusted_iam_role_arn` parameter).",
    "AND eventname = 'AssumeRole'": "Keep only `AssumeRole` events.",
    "AND eventsource = 'sts.amazonaws.com'": "Keep only STS API calls.",
}
