...
'''
This module implements the query: secretsmanager_scp_resource_perimeter
'''
import logging
from typing import (
    Dict,
...
class secretsmanager_scp_resource_perimeter(Query):
...
You can use this query to accelerate implementation of the [**resource perimeter**](https://aws.amazon.com/blogs/security/establishing-a-data-perimeter-on-aws-allow-only-trusted-resources-from-my-organization/) controls using service control policies (SCPs).
    ...
    def generate_athena_statement(
        ...
        resource_perimeter_trusted_secret_arn = helper.get_athena_dph_configuration(
            account_id=account_id,
            configuration_key="resource_perimeter_trusted_secret_arn",
            column_name="JSON_EXTRACT_SCALAR(requestparameters, '$.secretId')",
            params=params,
            with_negation=True
        )
